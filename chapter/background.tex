...
\section{Knowledge Distillation} \label{knowledge distillation}
    \gls{kd} is a method within deep learning which focuses on transferring the knowledge from one \textit{teacher} network with strong capability to a \textit{student} network. The most common use is to train less complex student models suited to resource-limited platforms like mobile devices. However, \gls{kd} has also been applied to train models equal to or even more complex than the teacher network. \gls{kd} can also be applied to train student models with constrained, additional, or limited data and possibly many teacher networks \cite{abbasi2020modeling}. There have been several approaches within this field, which can be approximately divided into two sub-categories: those that only use the teacher's output labels\footnote{The teachers labels can either be binary classes values called \textit{hard} labels or the class probability distribution from the softmax function called \textit{soft} labels.} as true labels to train the student, and those that extract knowledge from the layers of the teacher and use this during training of the student with or without teacher labels. The various approaches show that student models can achieve the same or even better performance than the teacher, as detailed in  \citet{alkhulaifi2021knowledge}. 
