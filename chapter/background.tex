...
\subsection{Mini-batch Stochastic Gradient Descent} \label{batch learning}
    ...
    %Goodfellow-et-al-2016_SGD \cite{wilson2001need_learning_rate}
...
\section{Knowledge Distillation} \label{knowledge distillation}
  ...
  %The various approaches show that student models can achieve the same or even better performance than the teacher, as detailed in  \citet{alkhulaifi2021knowledge}.
    
    
    %which can be approximately divided into two sub-categories: those that only use the teacher's output labels as true labels to train the student, and those that extract knowledge from the layers of the teacher and use this during training of the student with or without teacher labels. 
    
    
%     - hardcore - offline


% - litt lettere, hard labels
%    \cite{gou2021knowledge}
%     - masse arbeider -applications
    
%https://www.ncbi.nlm.nih.gov/pmc/articles/PMC8053015/
